...
SELECT
  ...
  [HOSCFB]           AS [ＣＩＦ番号],
  CASE
   WHEN [TKTKOZKBN] LIKE '%特定口座%' THEN
        '3120200000000'
   ELSE '3120100000000' END AS [商品コード]
...
AND   ( [KZPBEE] = 0 OR 
        [KZPBEE] > ( SELECT [前日] FROM ${INFO_DB}.T_KT00060D_LOAD ) )
GROUP BY (SELECT [前日] FROM ${INFO_DB}.T_KT00060D_LOAD ), [HOSTBN], [HOSCFB], CASE WHEN [TKTKOZKBN] LIKE '%特定口座%' THEN '3120200000000' ELSE '3120100000000' END;
SELECT @ErrorCode = @@ERROR;
IF @ErrorCode <> 0 GOTO ENDPT;
/* ######################################################################### */
/*                               通常の退出処理                              */
