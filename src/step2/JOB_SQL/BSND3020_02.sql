...
# ==============================================================================
# 作成テーブル名称  :顧客商品契約有無＿階層３                | HN_INFO.T_SN30200D_WK2
# フェーズ          :階層３作成
# サイクル          :日次
...
/* このＳＱＬでは、そのうち階層３の商品コードを作成する                */
/* ****************************************************************** */

/* 階層３で突合 */
INSERT INTO ${INFO_DB}.T_SN30200D_WK2
SELECT
   0                           AS [作成基準日],
   [TBN]                       AS [店番],
   [CFB]                       AS [ＣＩＦ番号],
   [TWGCFB]                    AS [統合ＣＩＦ番号],
   SUBSTRING([SFM],1,5) + '00000000' AS [商品コード]
...
GROUP BY [TWGCFB],[TBN],[CFB],SUBSTRING([SFM],1,5) + '00000000';
SELECT @ErrorCode = @@ERROR;
IF @ErrorCode <> 0 GOTO ENDPT;
