...
# ==============================================================================
# 作成テーブル名称  :【管理店】手動設定管理店情報_バッチ       | HN_INFO.T_KT15510D_OBJ
# フェーズ          :共通処理
# サイクル          :日次
...
WITH RankedData AS (
 SELECT
       ...
       A.[管理店番],
       '01'         AS [管理店番チャネル区分],
       ROW_NUMBER() OVER (PARTITION BY B.[統合番号] ORDER BY A.[登録日] DESC, A.[店番], A.[ＣＩＦ番号], A.[管理店番] DESC) as ranking
 FROM (
    SELECT
          [登録日],
          [店番],
    ...
    WHERE [登録日] <= (SELECT [前日] FROM ${INFO_DB}.[T_KT00060D_LOAD])
    ) AS A
...
INSERT INTO ${INFO_DB}.[T_KT15510D_OBJ]
SELECT
  [統合番号],
  [店番],
  [ＣＩＦ番号],
