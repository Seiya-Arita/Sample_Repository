...
# ==============================================================================
# 作成テーブル名称  :融資計数ワーク 事業資金貸出(保証付)       | HN_INFO.T_KT12070D_WK22
# フェーズ          :ＣＩＦ残高
# サイクル          :日次
...
/* 融資計数ワーク 事業資金貸出(保証付) */
/* ****************************************************************** */
INSERT INTO ${INFO_DB}.T_KT12070D_WK22
SELECT
     MAX([基準日])       AS [作成基準日]
    ,A.[店番]            AS [店番]
    ,A.[ＣＩＦ番号]      AS [ＣＩＦ番号]
    ,''                  AS [科目]
    ,'4190000000000'     AS [商品コード]
    ...
    (SELECT
     ...
     WHERE
       ...
       (  [科目] IN ('51','52','53','57') AND 
          [法個人コード] NOT IN ('21','22','23','32') AND
          [付保証コード]  IN ('08','09')
          AND
          ( ( SUBSTRING([業種コード],1,3)<>'140' AND 
              [商品コード１] NOT IN ('01','02','03','04','05','06') )
            OR
            ( SUBSTRING([業種コード],1,3) ='140' )
          )
       )
       OR
       ( ( [科目]='57' AND [商品コード１] = '06' AND 
           [法個人コード] NOT IN ('21','22','23','32') 
            )
         OR
         ( [科目]='11' AND [法個人コード] NOT IN ('21','22','23','32') AND
           SUBSTRING([業種コード],1,3) <> '140' AND [付保証コード] IN ('08','09') )
         OR 
