...
# ==============================================================================
# 作成テーブル名称  :【管理店】取経先管理店情報_バッチ         | HN_INFO.T_KT15530D_OBJ
# フェーズ          :共通処理
# サイクル          :日次
# 参照テーブル      :【管理店】取経先管理店情報(前日)          | HN_INFO.T_SN02040D_001
...
(
   SELECT
      ...
      A.[管理店番],
      A.[取経区分],
      ROW_NUMBER() OVER (PARTITION BY B.[統合番号] ORDER BY B.[代表ＣＩＦフラグ] DESC, A.[店番], A.[ＣＩＦ番号], A.[管理店番] DESC) AS row_num
   FROM ${INFO_DB}.[T_SN02040D_001] AS A
   INNER JOIN ${INFO_DB}.[T_KT15580D_OBJ] AS B
   ON (A.[店番] = B.[店番]
   AND A.[ＣＩＦ番号] = B.[ＣＩＦ番号])
)
INSERT INTO ${INFO_DB}.[T_KT15530D_OBJ]
SELECT
      B.[統合番号],
      A.[店番],
      A.[ＣＩＦ番号],
