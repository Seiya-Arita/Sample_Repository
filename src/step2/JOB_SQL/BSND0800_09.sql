...
# =========================================================================
# 追加テーブル名称  :                   | T_SN00800D_WK2
# フェーズ          :テーブル作成
# サイクル          :日次
# 実行日            :
# 参照テーブル      :                   | T_SN00800D_WK1
...
/*  09_統合ＣＩＦ番号設定 */
/* ******************************************************/
INSERT INTO ${INFO_DB}.[T_SN00800D_WK2]
SELECT
    A.[作成基準日] AS [作成基準日],
    CASE WHEN B.[統合ＣＩＦ番号] IS NULL THEN '' ELSE B.[統合ＣＩＦ番号] END AS [統合ＣＩＦ番号],
    A.[店番] AS [店番],
    A.[ＣＩＦ番号] AS [ＣＩＦ番号],
    A.[科目] AS [科目],
    A.[口座番号] AS [口座番号],
...
FROM ${INFO_DB}.[T_SN00800D_WK1] A
LEFT JOIN ${INFO_DB}.[T_SN00803D_WK1] B
    ON A.[店番] = B.[店番]
    AND A.[ＣＩＦ番号] = B.[ＣＩＦ番号];
SELECT @ErrorCode = @@ERROR;
IF @ErrorCode <> 0 GOTO ENDPT;
