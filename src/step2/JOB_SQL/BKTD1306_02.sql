...
# ==============================================================================
# 作成テーブル名称  :預り資産計数集約(投信)                    | HN_INFO.T_KT13070D_WK02
# フェーズ          :共通処理
# サイクル          :日次
# 参照テーブル      :投信残高属性断面展開                      | HN_INFO.T_KT13140D_OBJ
...
/* 預り資産計数ワーク 当行投信 */
/* ****************************************************************** */
INSERT INTO ${INFO_DB}.[T_KT13070D_WK02]
SELECT
     MAX([基準日])       AS [作成基準日]
    ,A.[店番]            AS [店番]
    ,A.[ＣＩＦ番号]      AS [ＣＩＦ番号]
    ,'3050300000000'     AS [商品コード]
...
FROM
   (SELECT
     ...
     ,[引落ＣＩＦ番号]      [ＣＩＦ番号]
     ,[評価金額_約定_chya]  [残高]
    FROM ${INFO_DB}.[T_KT13140D_OBJ]
    WHERE [評価金額_約定_chya] > 0
      AND [引落店番] <> 0
