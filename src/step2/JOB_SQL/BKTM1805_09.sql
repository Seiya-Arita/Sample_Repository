...
FROM (
    ...
    CROSS JOIN  ${INFO_DB}.T_KT00060D_LOAD  AS B                 /* 日付テーブル */
        WHERE (A.[TAKKNB] BETWEEN B.[前月月初日] AND B.[前月末日])     /*前月分*/
          ...
          AND (A.[TKYCOD] BETWEEN '010' AND '199'
           OR  A.[TKYCOD] BETWEEN '200' AND '218'
           OR  A.[TKYCOD] BETWEEN '219' AND '249'
           OR  A.[TKYCOD] BETWEEN '565' AND '577'
           OR  A.[TKYCOD] BETWEEN '700' AND '720'
           OR  A.[TKYCOD] BETWEEN '800' AND '859')                /*摘要コード */
          AND  A.[BMPCOD] <> ''                                   /*ＢＭＰコード */
    ) AS C
