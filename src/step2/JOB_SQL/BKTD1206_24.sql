...
# ==============================================================================
# 作成テーブル名称  :融資計数ワーク 個人ローン合計             | HN_INFO.T_KT12070D_WK24
# フェーズ          :ＣＩＦ残高
# サイクル          :日次
...
/* 融資計数ワーク 個人ローン合計 */
/* ****************************************************************** */
INSERT INTO ${INFO_DB}.T_KT12070D_WK24
SELECT
     MAX([基準日])       AS [作成基準日]
    ,A.[店番]            AS [店番]
    ,A.[ＣＩＦ番号]      AS [ＣＩＦ番号]
    ,''                  AS [科目]
    ,'AZZZZZZZZ0024'     AS [商品コード]
...
FROM
    (SELECT
     ...
     WHERE
        (
         (  [科目] IN ('53','54') AND [商品コード１] IN ('01','02','03','04','05') )
        OR
         (  ([科目]='12' AND [種別コード＿預金]='02') 
             AND
             (
               SUBSTRING([カードローンコード],1,4) IN ('1001','1002','1003','1004')
               OR
               SUBSTRING([カードローンコード],1,4) BETWEEN '1006' AND '1059'
             )
         )
        )
