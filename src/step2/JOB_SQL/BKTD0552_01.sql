...
# ==============================================================================
# 作成テーブル名称  :【管理店】変更依頼管理店情報              | HN_INFO.T_KT05520D_OBJ
# フェーズ          :共通処理
# サイクル          :日次
...
SELECT
             ...
             [明細番号],
             [申請区分],
             ROW_NUMBER() OVER (PARTITION BY B.[統合番号] ORDER BY A.[変更希望日] DESC, A.[管理番号] DESC, A.[明細番号] DESC, A.[店番], A.[ＣＩＦ番号]) AS row_num
       FROM [${INFO_DB}].[T_SN02080D_OBJ]
       WHERE [変更希望日] <= (SELECT [基準日] FROM [${INFO_DB}].[T_KT00060D_LOAD])
         AND [ジョブ状態] = '承認'
         AND [無効フラグ] = '0'
       ) AS A
...
INSERT INTO [${INFO_DB}].[T_KT05520D_OBJ]
SELECT
      D.[統合番号],
      D.[店番],
      D.[ＣＩＦ番号],
