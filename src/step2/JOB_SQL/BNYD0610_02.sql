...
# サイクル          :日次
# 実行日            :
# 参照テーブル      :世帯名寄せ採番				| T_NY06100D_OBJn
#                   :名寄せフォーマット変換		| T_NY04000D_OBJ
#                   :最大世帯番号				| T_NY06101D_WK1
...
DELETE FROM ${INFO_DB}.[T_NY06101D_WK1];
SELECT @ErrorCode = @@ERROR;
IF @ErrorCode <> 0 GOTO ENDPT;
INSERT INTO ${INFO_DB}.[T_NY06101D_WK1]
SELECT
  COALESCE(MAX([世帯番号]),0) AS [最大世帯番号]
FROM ${INFO_DB}.[T_NY06100D_${EXT_ID}];
SELECT @ErrorCode = @@ERROR;
IF @ErrorCode <> 0 GOTO ENDPT;
...
CROSS JOIN ${INFO_DB}.[T_NY06101D_WK1] AS B
INNER JOIN (
