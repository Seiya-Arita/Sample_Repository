...
# ==============================================================================
# 作成テーブル名称  :預金ＣＩＦ基本断面展開                    | HN_INFO.T_KT10100D_OBJ
# フェーズ          :前処理
# サイクル          :日次
# 参照テーブル      :預金ＣＩＦ基本S                           | HN_V_SRC.T_YK50060D_SRC001
...
/* ****************************************************************** */
/* 預金ＣＩＦ基本断面展開
/* 期初日～最新断面(前日)の断面を展開したOBJを作成
/* ****************************************************************** */
INSERT INTO ${INFO_DB}.[T_KT10100D_OBJ]
SELECT B.[基準日],A.[TBN],A.[CFB],A.[SSSHYJ],A.[HJKCOD],A.[GYSCOD] FROM
  (SELECT [start_date],[end_date],[TBN],[CFB],[SSSHYJ],[HJKCOD],[GYSCOD]
   FROM ${DB_T_SRC}.[T_YK50060D_SRC001] WHERE [Record_Deleted_Flag] = 0
