...
# =========================================================================
# 追加テーブル名称  :                   | T_SN00801D_WK3
# フェーズ          :テーブル作成
# サイクル          :日次
# 実行日            :
# 参照テーブル      :                   | T_SK51040D_SRC001
#                                           ---> T_SK51110D_SRC001
...
/*  05_ＮＩＳＡ契約決済口座取得 */
/* ******************************************************/
INSERT INTO ${INFO_DB}.T_SN00801D_WK3
SELECT
     A.[HOSTBN]                            AS [店番],
     A.[HOSCFB]                            AS [ＣＩＦ番号],
     A.[HOSKMK]                            AS [科目],
     A.[HOSKZB]                            AS [口座番号]
/*  FROM       ${DB_T_SRC}.T_SK51040D_SRC001  A                   2024-01-25  */
...
/*      AND  A.MAKKJB = B.前日                                    2024-01-25  */
/*      AND  A.TKTAZKKBN LIKE '%ＮＩＳＡ%'                        2024-01-25  */
/******************************************************************************/
/*          ( Add extraction condition )                          2024-01-25  */
  AND A.[N1SKYAKBN] LIKE '%契約%'
  AND A.[N1SBUBKBNTWN] LIKE '%可能%'
/******************************************************************************/
  AND A.[HOSTBN] <> 0
  AND A.[HOSCFB] <> 0
  AND A.[HOSKMK] <> ''
