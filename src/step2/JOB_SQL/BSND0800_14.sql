...
# =========================================================================
# 追加テーブル名称  :                   | T_SN00800D_WK7
# フェーズ          :テーブル作成
# サイクル          :日次
# 実行日            :
# 参照テーブル      :                   | T_SN00800D_WK6
...
/*  14_ＮＩＳＡ契約あり＿銀行契約設定 */
/* ******************************************************/
INSERT INTO ${INFO_DB}.[T_SN00800D_WK7]
SELECT
    A.[作成基準日]                         AS [作成基準日],
    ...
    CASE WHEN A.[店番] = B.[店番]
      ...
      THEN '1'
      ELSE '0'
    END                                  AS [ＮＩＳＡ契約あり＿銀行契約],
    A.[ＮＩＳＡ契約あり＿ＦＧ契約]         AS [ＮＩＳＡ契約あり＿ＦＧ契約]
FROM      ${INFO_DB}.[T_SN00800D_WK6]  A
LEFT JOIN ${INFO_DB}.[T_SN00801D_WK2]  B
   ON  A.[店番] = B.[店番]
   AND A.[科目] = B.[科目]
   AND A.[口座番号] = B.[口座番号];
SELECT @ErrorCode = @@ERROR;
