...
# ==============================================================================
# 作成テーブル名称  :【管理店】取経先管理店情報                | HN_INFO.T_KT05530D_OBJ
# フェーズ          :共通処理
# サイクル          :日次
# 参照テーブル      :【管理店】取経先管理店情報                | HN_INFO.T_SN02040D_LOAD
...
(
   SELECT
          ...
          [管理店番],
          [取経区分],
          ROW_NUMBER() OVER (PARTITION BY B.[統合番号]
            ORDER BY B.[代表ＣＩＦフラグ] DESC, A.[店番], A.[ＣＩＦ番号], A.[管理店番] DESC) as row_num
...
INSERT INTO ${INFO_DB}.[T_KT05530D_OBJ]
SELECT
      B.[統合番号],
      A.[店番],
      A.[ＣＩＦ番号],
