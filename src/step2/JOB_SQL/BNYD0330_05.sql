...
# ==============================================================================
# 作成テーブル名称  :機能サービス数                            | HN_INFO.T_NY03000D_WK9
# フェーズ          :機能サービス数
# サイクル          :日次
...
/* 機能サービス数 */
/* ****************************************************************** */
INSERT INTO ${INFO_DB}.[T_NY03000D_WK9]
SELECT
   A.[作成基準日]                        AS [作成基準日],
   A.[店番]                              AS [店番],
   A.[ＣＩＦ番号]                        AS [ＣＩＦ番号],
...
FROM (
       ...
       WHERE CAST(CAST(T2.[前前日] AS CHAR(8)) AS DATE) BETWEEN T1.[Start_Date] AND T1.[End_Date]
         AND T1.[Record_Deleted_Flag]=0
         AND (
               /* 機能サービス */
               (SUBSTRING(T1.[SFM],1,1) = '7')
                OR
               /* 自動支払(公共料金除く)*/
               (SUBSTRING(T1.[SFM],1,1) = '6' AND SUBSTRING(T1.[SFM],1,3) <> '601')
             )
     ) AS A
     INNER JOIN ${DB_T_SEM}.[V_KT41000D_NYS] AS B
     ON A.[商品コード] = B.[SHNCOD]
GROUP BY A.[作成基準日],A.[店番],A.[ＣＩＦ番号];
SELECT @ErrorCode = @@ERROR;
IF @ErrorCode <> 0 GOTO ENDPT;
/* ######################################################################### */
/*                               通常の退出処理                              */
