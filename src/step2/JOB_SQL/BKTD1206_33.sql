...
# ==============================================================================
# 作成テーブル名称  :融資計数ワーク 事業者カードローン         | HN_INFO.T_KT12070D_WK33
# フェーズ          :ＣＩＦ残高
# サイクル          :日次
...
/* 融資計数ワーク 事業者カードローン */
/* ****************************************************************** */
INSERT INTO ${INFO_DB}.T_KT12070D_WK33
SELECT
     MAX([基準日])       AS [作成基準日]
    ,A.[店番]            AS [店番]
    ,A.[ＣＩＦ番号]      AS [ＣＩＦ番号]
    ,''                  AS [科目]
    ,'4130000000000'     AS [商品コード]
...
FROM
    (SELECT
     ...
     WHERE
        (
            [科目]='12'
        AND [法個人コード] NOT IN ('21','22','23','32')
        AND SUBSTRING([カードローンコード],1,4)='1005'
        )
