...
# ==============================================================================
# 作成テーブル名称  :融資計数ワーク カードローン               | HN_INFO.T_KT12070D_WK25
# フェーズ          :ＣＩＦ残高
# サイクル          :日次
...
/* 融資計数ワーク カードローン */
/* ****************************************************************** */
INSERT INTO ${INFO_DB}.T_KT12070D_WK25
SELECT
     MAX([基準日])       AS [作成基準日]
    ,A.[店番]            AS [店番]
    ,A.[ＣＩＦ番号]      AS [ＣＩＦ番号]
    ,''                  AS [科目]
    ,'4010000000000'     AS [商品コード]
...
FROM
    (SELECT
     ...
     WHERE
        [科目] = '12'
        AND [種別コード＿預金] = '02'
        AND SUBSTRING([カードローンコード],1,4) IN
           ('1004','1002','1001','1006','1003','1007')
