...
# ==============================================================================
# 作成テーブル名称  :【顧客商品有無】損保                       | HN_INFO.T_SN25500D_OBJ
# フェーズ          :損保商品契約有無を判定
# サイクル          :日次
...
FROM
  ( SELECT
       ...
       [CIFKYABNG]      AS [ＣＩＦ番号],
       CASE
         WHEN [HENSMO] = '01' THEN '01'
         WHEN [HENSMO] = '04' THEN '02'
         ELSE '03'
       END            AS [コード階層区分３],
...
GROUP BY C.[前日], A.[店番], A.[ＣＩＦ番号], COALESCE(B.[業務商品コード],'904' + A.[コード階層区分３] + '00000000');
SELECT @ErrorCode = @@ERROR;
IF @ErrorCode <> 0 GOTO ENDPT;
/* ######################################################################### */
/*                               通常の退出処理                              */
