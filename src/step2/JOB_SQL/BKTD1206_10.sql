...
# ==============================================================================
# 作成テーブル名称  :融資計数ワーク  貸出合計                  | HN_INFO.T_KT12070D_WK10
# フェーズ          :ＣＩＦ残高
# サイクル          :日次
...
/* ****************************************************************** */
/* 融資計数ワーク 貸出合計
/* 商業手形+手形貸付+証書貸付+融資当貸+預金当貸+外貨貸出+ローン
/* ****************************************************************** */
INSERT INTO ${INFO_DB}.T_KT12070D_WK10
SELECT
    MAX(ZN.[作成基準日])              AS [作成基準日]
   ,ZN.[店番]                         AS [店番]
   ,ZN.[ＣＩＦ番号]                   AS [ＣＩＦ番号]
   ,''                                AS [科目]
   ,'AZZ0000000010'                   AS [商品コード]
   ,SUM(ZN.[残高])                    AS [残高]
   ,SUM(ZN.[月中積数])                AS [月中積数]
   ,SUM(ZN.[月中平残])                AS [月中平残]
   ,SUM(ZN.[期中積数])                AS [期中積数]
...
(
   ...
   SELECT * FROM ${INFO_DB}.T_KT12070D_WK06
) AS ZN
