...
# ==============================================================================
# 作成テーブル名称  :【管理店】貸出先管理店情報_バッチ         | HN_INFO.T_KT15540D_OBJ
# フェーズ          :共通処理
# サイクル          :日次
# 参照テーブル      :ＣＩＦ残高融資                            | HN_INFO.T_KT12100D_OBJ
#                   :【管理店】管理店名寄せ情報                | HN_INFO.T_KT15580D_OBJ
#                   :【管理店】貸出先管理店情報（前日）        | HN_INFO.T_KT15540D_001
#                   :【管理店】統合番号親ＣＩＦ                | HN_INFO.T_KT15541D_WK
#
# 変更履歴
# ------------------------------------------------------------------------------
...
/* ****************************************************************** */
/* 【管理店】貸出先管理店情報 作成、抽出（住宅ローン） */
/* ****************************************************************** */
INSERT INTO ${INFO_DB}.[T_KT15540D_OBJ]
SELECT
     H.[前日]                 AS [作成基準日],
     C.[統合番号],
     E.[店番],
...
LEFT JOIN (
    SELECT
          [統合番号],
          [管理店番]
      FROM ${INFO_DB}.[T_KT15540D_001]
     WHERE [作成基準日] = (SELECT [前前日] FROM ${INFO_DB}.[T_KT00060D_LOAD])
       AND [管理店番チャネル区分] = '05'
     ) AS D
ON (C.[統合番号] = D.[統合番号])
INNER JOIN ${INFO_DB}.[T_KT15541D_WK] AS E
ON (C.[統合番号] = E.[統合番号])
